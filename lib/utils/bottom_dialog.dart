...
class CommonBottomSheet extends StatefulWidget {
  CommonBottomSheet(
      {required Key key, this.list, required this.onItemClickListener})
      : assert(list != null),
        super(key: key);
  final list;
...
typedef OnItemClickListener = void Function(num index);
...
class _CommonBottomSheetState extends State<CommonBottomSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    var height = ((widget.list.length + 1) * 50 + 20);
  ...
  Widget getItemContainer(BuildContext context, index) {
    if (widget.list == null) {
      return Container();
    }
    if (index.isOdd) {
